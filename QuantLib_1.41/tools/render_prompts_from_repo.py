...
def _find_file(ql_dir: str, rel_path: str) -> str | None:
    ...
    if rel_path.startswith(("ql/", "ql\\")):
        c = os.path.join(base, rel_path[3:])
        if os.path.isfile(c):
            return c
...
def _extract_rounds(doc: str) -> dict[int, dict]:
    ...
    for section in re.split(r"\n\s*-{3,}\s*\n", doc):
        hdr = re.search(
            r"###\s+(?:Optional\s+)?ROUND\s+(\d+)\s*[-—–]+\s*(.+)", section
        )
        if not hdr:
            continue
        num = int(hdr.group(1))
        ...
        if xml_block:
            rounds[num] = dict(title=hdr.group(2).strip(),
...
def _stage_standalone(content: str, ql_dir: str, verbose: bool) -> str:
    ...
    for m in _RE_INSERT.finditer(content):
        ...
        # (g) Unrecognised — leave as-is with a warning
        else:
            print(
